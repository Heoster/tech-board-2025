...
const AdminDashboard: React.FC = () => {
  ...
  const [questionStats, setQuestionStats] = useState({ total: 0, byGrade: {} });
    ...
    const fetchData = async () => {
      try {
        const [studentsResponse, questionsResponse] = await Promise.all([
          ...
          axios.get('/api/admin/question-counts')
        ]);
        ...
        if (questionsResponse.data.success) {
          ...
          const total = Object.values(questionData).reduce((sum: number, grade: any) => sum + grade.current, 0);
          setQuestionStats({ total, byGrade: questionData });
        }
      } catch (error) {
      ...
      <div>
        ...
        <div className="grid md:grid-cols-6 gap-6 mb-8">
          <div className="card text-center hover:scale-105 transform transition-all duration-300 animate-fade-in">
            ...
            <h3 className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{questionStats.total}</h3>
