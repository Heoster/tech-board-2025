...
import { Clock, ChevronLeft, ChevronRight, Flag, CheckCircle, AlertTriangle, BookOpen, Target, Timer } from 'lucide-react';
...
const QuizInterface = () => {
  ...
  const handleSubmit = async (autoSubmit = false) => {
    ...
    try {
      ...
      // Prepare answers in the expected format (include all 50 questions)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow-xl p-8 text-center max-w-md">
          ...
          <p className="text-gray-600 mb-4">Loading questions for your grade...</p>
          {user && (
            <div className="bg-blue-50 rounded-lg p-3">
              <p className="text-sm text-blue-700">
                <BookOpen className="inline w-4 h-4 mr-1" />
                Grade: {user.grade} | Student: {user.name}
              </p>
            </div>
          )}
        </div>
                ...
                <div className="space-y-4 mb-8">
                  {questions[currentQuestion]?.options.map((option, index) => (
                    <button
                      ...
                      className={`w-full p-5 text-left rounded-xl border-2 transition-all duration-200 transform hover:scale-[1.02] ${answers[currentQuestion] === option.id
                          ? 'border-blue-500 bg-gradient-to-r from-blue-50 to-blue-100 text-blue-900 shadow-md'
                          : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50 hover:shadow-sm'
                        }`}
                    >
                      <div className="flex items-center space-x-4">
                        <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all ${answers[currentQuestion] === option.id
                        ...
                        <div className="flex items-center space-x-3 flex-1">
                          <span className={`font-bold text-lg ${answers[currentQuestion] === option.id ? 'text-blue-700' : 'text-gray-600'}`}>
                            {String.fromCharCode(65 + index)}.
                          </span>
                          <span className={`text-base leading-relaxed ${answers[currentQuestion] === option.id ? 'font-medium' : ''}`}>
                            {option.text}
                          </span>
                        </div>
                      </div>
