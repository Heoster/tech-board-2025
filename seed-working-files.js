...
async function seedWorkingFiles() {
    console.log('🌱 Starting database seeding from working question files...');
                ...
                db.exec(schema, async (err) => {
                    ...
                    try {
                        ...
                        db.run('INSERT INTO admins (username, password) VALUES (?, ?)', 
                            ...
                            // Load and seed questions from working files
                            seedFromWorkingFiles(db, () => {
                                db.close((err) => {
                                    if (err) {
...
function seedFromWorkingFiles(db, callback) {
    console.log('📝 Loading questions from working files...');
    
    // Use the working files that we know exist
    const questionFiles = [
        { file: 'grade6.js', grade: 6 },
        { file: 'grade7.js', grade: 7 },
        { file: 'grade8.js', grade: 8 },
        { file: 'grade9.js', grade: 9 },
        { file: 'grade11.js', grade: 11 }
    ];
                ...
                db.run('INSERT OR IGNORE INTO questions (grade, difficulty, question_text) VALUES (?, ?, ?)',
                    ...
                    if (!questionId) {
                        ...
                        if (questionsProcessed === questions.length) {
                            console.log(`✅ Grade ${grade} completed: ${questions.length} questions processed`);
                            filesProcessed++;
                            if (filesProcessed === questionFiles.length) {
...
if (require.main === module) {
    seedWorkingFiles().catch(error => {
        console.error('❌ Seeding failed:', error.message);
        process.exit(1);
    });
}

module.exports = { seedWorkingFiles };