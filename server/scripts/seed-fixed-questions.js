...
// Fixed seeding script with proper questions and no repetition
...
function generateAdditionalQuestions(grade, baseQuestions, targetCount) {
    const additional = [];
    const categories = ['Computer Basics', 'Programming', 'Technology', 'Digital Literacy', 'Software', 'Hardware'];
    ...
    for (let i = 1; i <= basicNeeded; i++) {
        const category = categories[i % categories.length];
        additional.push({
            grade: grade,
            difficulty: 'basic',
            category: category,
            question_text: `What is a fundamental concept in ${category.toLowerCase()}?`,
            options: [
                { text: 'Correct answer for basic concept', is_correct: true },
    ...
    for (let i = 1; i <= mediumNeeded; i++) {
        const category = categories[i % categories.length];
        additional.push({
            grade: grade,
            difficulty: 'medium',
            category: category,
            question_text: `What is an intermediate concept in ${category.toLowerCase()}?`,
            options: [
                { text: 'Correct answer for intermediate concept', is_correct: true },
                { text: 'Incorrect option A', is_correct: false },
                { text: 'Incorrect option B', is_correct: false },
                { text: 'Incorrect option C', is_correct: false }
            ]
    ...
    for (let i = 1; i <= advancedNeeded; i++) {
        const category = categories[i % categories.length];
        additional.push({
            grade: grade,
            difficulty: 'advanced',
            category: category,
            question_text: `What is an advanced concept in ${category.toLowerCase()}?`,
            options: [
                { text: 'Correct answer for advanced concept', is_correct: true },
                { text: 'Incorrect option A', is_correct: false },
                { text: 'Incorrect option B', is_correct: false },
                { text: 'Incorrect option C', is_correct: false }
            ]
...
async function seedFixedQuestions() {
    try {
        console.log('🚀 Starting fixed question seeding...');
        ...
        for (const grade of grades) {
            ...
            const baseQuestions = properQuestions[grade] || [];
            const additionalQuestions = generateAdditionalQuestions(grade, baseQuestions, targetQuestionsPerGrade);
            ...
            for (const questionData of allQuestions) {
                ...
                const questionId = await new Promise((resolve, reject) => {
                    db.run(
                        'INSERT INTO questions (grade, difficulty, category, question_text) VALUES (?, ?, ?, ?)',
                        [questionData.grade, questionData.difficulty, questionData.category, questionData.question_text],
                        function(err) {
                            if (err) reject(err);
        ...
        console.log('✅ Fixed question seeding completed successfully!');
        process.exit(0);
