    ...
    try {
        ...
        const stats = await Promise.all([
            ...
            new Promise((resolve, reject) => {
                db.get('SELECT COUNT(*) as count FROM students', (err, row) => {
                    if (err) reject(err);
                    else resolve({ totalStudents: row.count });
                });
            }),
            ...
            new Promise((resolve, reject) => {
                db.get('SELECT COUNT(*) as count FROM questions', (err, row) => {
                    if (err) reject(err);
                    else resolve({ totalQuestions: row.count });
                });
            }),
            ...
            new Promise((resolve, reject) => {
                db.get('SELECT COUNT(*) as count FROM quizzes WHERE status = "completed"', (err, row) => {
                    if (err) reject(err);
                    else resolve({ completedQuizzes: row.count });
                });
            }),

            // In progress quizzes
            new Promise((resolve, reject) => {
                db.get('SELECT COUNT(*) as count FROM quizzes WHERE status = "in_progress"', (err, row) => {
                    if (err) reject(err);
                    else resolve({ inProgressQuizzes: row.count });
                });
            }),

            // Average score
            new Promise((resolve, reject) => {
                db.get('SELECT AVG(CAST(score AS FLOAT) / total_questions * 100) as avg_score FROM quizzes WHERE status = "completed"', (err, row) => {
                    if (err) reject(err);
                    else resolve({ averageScore: row.avg_score ? Math.round(row.avg_score * 10) / 10 : 0 });
                });
            }),
            ...
            new Promise((resolve, reject) => {
                db.all('SELECT grade, COUNT(*) as count FROM questions GROUP BY grade ORDER BY grade', (err, rows) => {
                    if (err) reject(err);
                        ...
                        rows.forEach(row => {
                            questionsByGrade[row.grade] = row.count;
                        });
                        resolve({ questionsByGrade });
                    }
                });
            })
        ]);
        ...
        // Combine all stats
        const systemStats = stats.reduce((acc, stat) => ({ ...acc, ...stat }), {});
...
// Get system statistics (fix the missing endpoint)
router.get('/system-stats', authenticateToken, requireAdmin, validateAdmin, async (req, res) => {
    try {
        const db = database.getDb();
