...
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_quiz_question_response 
...
-- Additional constraint: Ensure quiz has exactly 25 questions
...
BEGIN
    -- Check if quiz has exactly 25 responses
    SELECT CASE 
        WHEN (SELECT COUNT(*) FROM responses WHERE quiz_id = NEW.id) != 25
        THEN RAISE(ABORT, 'Quiz must have exactly 25 responses')
    END;
    
    -- Check for duplicate questions in the quiz
    SELECT CASE 
        WHEN (SELECT COUNT(DISTINCT question_id) FROM responses WHERE quiz_id = NEW.id) != 25
...
BEGIN
    ...
    SELECT CASE 
        WHEN EXISTS (
            ...
            AND q.student_id = (
                SELECT student_id FROM quizzes WHERE id = NEW.quiz_id
            )
        )
        THEN RAISE(ABORT, 'Student has already answered this question in a previous quiz')
    END;
    ...
    SELECT CASE 
        WHEN EXISTS (
            ...
            WHERE quiz_id = NEW.quiz_id AND question_id = NEW.question_id
        )
...
CREATE INDEX IF NOT EXISTS idx_student_quiz_lookup 
ON quizzes(student_id, grade, status);

CREATE INDEX IF NOT EXISTS idx_response_question_lookup 
ON responses(question_id, quiz_id);
...
SELECT 
    ...
    CASE 
        ...
        WHEN COUNT(r.id) != 25 AND q.status = 'completed' THEN 'WRONG_QUESTION_COUNT'
        ELSE 'OK'
    END as integrity_status
FROM quizzes q
LEFT JOIN responses r ON q.id = r.quiz_id
GROUP BY q.id, q.student_id, q.grade, q.status;
...
GROUP BY s.id, s.name, s.grade, r.question_id
HAVING COUNT(*) > 1;
...
) used_q ON s.id = used_q.student_id AND q.id = used_q.question_id
...
SELECT 
    'STUDENT_REPETITION' as check_type,
    COUNT(*) as violation_count,
...
SELECT 
    'RESPONSE_COUNT' as check_type,
    COUNT(*) as violation_count,
    'Completed quizzes without exactly 25 responses' as description
FROM quizzes q
WHERE q.status = 'completed' 
AND (SELECT COUNT(*) FROM responses WHERE quiz_id = q.id) != 25;