...
const finalBatchMediumQuestions = [
    // Grade 6 Final Batch Medium Questions
    {
        grade: 6,
        questions: [
    ...
    // Grade 7 Final Batch Medium Questions
    {
        grade: 7,
        questions: [
    ...
    // Grade 8 Final Batch Medium Questions
    {
        grade: 8,
        questions: [
    ...
    // Grade 9 Final Batch Medium Questions
    {
        grade: 9,
        questions: [
    ...
    // Grade 11 Final Batch Medium Questions
    {
        grade: 11,
        questions: [
            { question: "What is the difference between GET and POST methods?", options: ["GET is read, POST is write", "POST is read, GET is write", "Both are read", "Both are write"], correct: 0, difficulty: "medium" },
...
async function addFinalBatchMediumQuestions() {
    try {
        console.log('Starting to add final batch medium questions...');
        let totalAdded = 0;

        for (const gradeData of finalBatchMediumQuestions) {
            console.log(`\nAdding final batch medium questions for Grade ${gradeData.grade}...`);
            let gradeAdded = 0;
            ...
            console.log(`✅ Added ${gradeAdded} final batch medium questions for Grade ${gradeData.grade}`);
        }

        console.log(`\n🎉 Successfully added ${totalAdded} final batch medium questions total!`);
        await showFinalStatistics();
        
    } catch (error) {
        console.error('Error adding final batch medium questions:', error);
    } finally {
        db.close((err) => {
            if (err) {
